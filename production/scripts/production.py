...
TEMPERATURE="../obj/temperature.csv"
SEASONAL_PRODUCTION="ca_seasonal_production.csv"
...
# constants
MIN_TEMP=21.8  # temperature interval for which regression with precipitation and temperature can be applied
MAX_TEMP=28.2
...
def MYSProd(annualProd,cellsToIgnore,eggFrac):
   ...
   # obtain state-wise scaling factors based on vegetable production
   regionalProd={}
   c.execute("SELECT * FROM vegetable_production_MYS")
   queryResult=c.fetchall()
   for x in queryResult:
      regionalProd[x[0]]=x[1]
   con.close()
   totRegionalProd=sum(regionalProd.values())
   ...
   for cell in annualProd:
      if "MY-" in annualProd[cell]['iso']:
         div=annualProd[cell]['admin']
         cellsToIgnore.append(cell)
         totProd[div]+=annualProd[cell]['vege']
   ...
   for cell in annualProd:
      if "MY-" in annualProd[cell]['iso']:
         div=annualProd[cell]['admin']
         try:
            annualProd[cell]['tomato']=tomFrac*regionalProd[div]/totProd[div]\
                     *annualProd[cell]['vege']*MYS_TOMATO/totRegionalProd
         except ZeroDivisionError:
            annualProd[cell]['tomato']=0
         try:
            annualProd[cell]['eggplant']=eggFrac*regionalProd[div]/totProd[div]\
                     *annualProd[cell]['vege']
         except ZeroDivisionError:
            annualProd[cell]['eggplant']=0
         annualProd[cell]['potato']=annualProd[cell]['potato']
...
def monthlyProdByPrecip(annualProd,mode):
   monthlyProd={}
   # read precipitation
   precipitation=pd.read_csv(PRECIPITATION,index_col='cell_id')
   # read temperature
   temperature=pd.read_csv(TEMPERATURE,index_col='cell_id')

   # disaggregation functions by mode
   uniformMonthlyProd=lambda: 1/12.0
   precip1MonthlyProd=lambda p: math.exp(-.208 -.008*p)
   precip2MonthlyProd=lambda p: math.exp(-.208 -.002*p)
   precipTempMonthlyProd=lambda p,t: math.exp(-6.360-0.008*p+.234*t)
   ...
   for cell in annualProd.keys():
      ...
      precip=precipitation.loc[cell][4:]
      temp=temperature.loc[cell][2:]
      quartPrecip=[sum(precip[i*3:(i+1)*3])/3.0 for i in xrange(4)]
      quartTemp=[sum(temp[i*3:(i+1)*3])/3.0 for i in xrange(4)]

      for i in xrange(4):
         if mode=="uniform":
            monthlyProd[cell]['tomato'][i*3:(i+1)*3]=[uniformMonthlyProd()]*3
            monthlyProd[cell]['eggplant'][i*3:(i+1)*3]=[uniformMonthlyProd()]*3
            monthlyProd[cell]['potato'][i*3:(i+1)*3]=[uniformMonthlyProd()]*3
         elif mode=="precip1":
            monthlyProd[cell]['tomato'][i*3:(i+1)*3]=[precip1MonthlyProd(quartPrecip[i])]*3
            monthlyProd[cell]['eggplant'][i*3:(i+1)*3]=[precip1MonthlyProd(quartPrecip[i])]*3
            monthlyProd[cell]['potato'][i*3:(i+1)*3]=[precip1MonthlyProd(quartPrecip[i])]*3
         elif mode=="precip2":
            monthlyProd[cell]['tomato'][i*3:(i+1)*3]=[precip2MonthlyProd(quartPrecip[i])]*3
            monthlyProd[cell]['eggplant'][i*3:(i+1)*3]=[precip2MonthlyProd(quartPrecip[i])]*3
            monthlyProd[cell]['potato'][i*3:(i+1)*3]=[precip2MonthlyProd(quartPrecip[i])]*3
         elif mode=="precip_temp":
            if not min(temp)<-1000: # temperature for the cell not provided
               monthlyProd[cell]['tomato'][i*3:(i+1)*3]=[precipTempMonthlyProd(quartPrecip[i],quartTemp[i])]*3
               monthlyProd[cell]['eggplant'][i*3:(i+1)*3]=[precipTempMonthlyProd(quartPrecip[i],quartTemp[i])]*3
               monthlyProd[cell]['potato'][i*3:(i+1)*3]=[precipTempMonthlyProd(quartPrecip[i],quartTemp[i])]*3
            else:
               monthlyProd[cell]['tomato'][i*3:(i+1)*3]=[precip1MonthlyProd(quartPrecip[i])]*3
               monthlyProd[cell]['eggplant'][i*3:(i+1)*3]=[precip1MonthlyProd(quartPrecip[i])]*3
               monthlyProd[cell]['potato'][i*3:(i+1)*3]=[precip1MonthlyProd(quartPrecip[i])]*3
         else:
            logging.error("Wrong mode '%s'." %mode)
...
if __name__ == "__main__":
   ...
   parser.add_argument("-m","--mode",help="function used for disaggregating \
         annual production (uniform/precip/precip_temp)",default="uniform")
   ...
   monthlyProduction=monthlyProdByPrecip(annual,args.mode)
