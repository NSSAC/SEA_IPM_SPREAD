...
desc/.style={inner sep=1pt,text width=3cm,font=\small},
...
%% grid
\node[blk] (abm) {\textbf{\large Network \\ dynamical system}};
\node[blk,right of=abm,shift={(.7,0)}] (explore) {\textbf{\large Explore\\ parameter space}};
\draw[blkedge,<->] (abm) -- (explore);
\node[measure,above of=explore,shift={(1,-.5)}] (score) {Loss function};
\draw[datedge,->] (score.south) -- +(0,-.3) -| ($(explore.north)+(.5,0)$);
\node[above of=explore,dat,shift={(-1,-.5)}] (gt) {Ground truth};
\draw[datedge,->] (gt.south) -- +(0,-.3) -| ($(explore.north)+(-.5,0)$);
%%
...
%%
\node[blk,right of=cluster,shift={(.7,0)}] (analyze) {\textbf{\large Analyze clusters}};
\draw[blkedge,->] (cluster) -- (analyze);
\node[alg,above of=analyze,shift={(0,-.5)}] (cart) {Supervised learning};
\draw[datedge,->] (cart) -- (analyze);

%%configurations etc.
\node[desc,below of=abm,shift={(0,.5)}] (config) {Configuration space $\mathcal{C}$};
\node[desc,below of=explore,shift={(0,.5)}] (select) {Configurations that
fit the ground truth $\mathcal{C}_s$ and corresponding set of simulation
outputs $\mathcal{S}_s$};
