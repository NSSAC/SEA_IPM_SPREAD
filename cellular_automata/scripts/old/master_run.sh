...
mooreList="2 3"
suitThreshList="0"
expDelayList=`seq 2 3`
alphaSDList=`seq 100 100 200` 
alphaFMList=`seq 0 0` 
alphaMMList=`seq 100 100 200`
...
alpha_sd=$alphaSD,\
alpha_fm=$alphaFM,\
alpha_mm=$alphaMM ../scripts/run_ca_BGD.sbatch
qreg
EOF
}
...
fList=eval_list.txt
rm -f $fList
for f in `ls -1 results_BGD/res_*csv`
do
for t in `seq 2 2`
do
    ...
    echo $logFile >> $fList
    sbatch -o $logFile \
        --export=command="python ../scripts/evaluate_probs.py --time_window $t $f" \
