...
function create_ca() {
#sbatch --export=out_file=$outFile,radius=$radius,cityFile=$cityFile ../scripts/create_ca.sbatch;
python ../scripts/create_ca.py -o $minCA -r $radius --city_file $cityFile;
}

function fill_data() {
#outFile=`echo $caFile | sed -e 's/.pkl/_prod.pkl/'`
python ../scripts/fill_data_ca.py --temp --hum --pop --prod --country --grid_file $minCA -o $minCAProd
# inputs to filter_mapspam: mapspam file (hardcoded), -p pickle file
}

function ann_prod(){
python ../scripts/filter_mapspam.py -p $minCAProd
# output file is ../obj/ca_mapspam_pop.csv
}

function seasonal_production() {
cd ../../production/work
../scripts/master_production.sh gen_prod
#python ../scripts/production.py
}

function locality_data() {
python ../scripts/locality_data.py \
    -p $minCAProd \
...
#../scripts/master_flow.sh gen_distance_matrix
../scripts/master_flow.sh gravity_flows
...
cd ../../long_distance/work/
#../scripts/master_flow.sh gen_distance_matrix
../scripts/master_flow.sh concat_flows
# output is ../../long_distance/obj/locality_flows_b${beta}_k${kappa}.csv
}
...
sbatch --account ipmmodeling -o $logFile \
    --export=command="python ../scripts/fill_data_ca.py --net --net_file $flows \
    --season_file $seasonalProd \
    --grid_file $minCA \
    --loc_file $locFile \
    -o $caFile" ../scripts/run_proc.sbatch
done
}
...
sbatch --account ipmmodeling -o $logFile \
    --export=command="python ../scripts/interventions.py --grid_file $f \
