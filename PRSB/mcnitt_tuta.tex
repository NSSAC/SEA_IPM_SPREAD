...
%% Bangladesh.} 
\paragraph{The multi-pathway spread in Bangladesh: Multiple possibilities.} As described in Methods, the goodness of fit of a model was
...
the popular K-means algorithm. It uses Bayesian information criterion to
estimate the optimal number of clusters. To discover the relationship between spread
...
entire process and details of the clustering analysis is in
...
the set of instances with similarity~$\similarity>6$. The results are in
...
\begin{subfigure}[b]{.45\textwidth}
    \includegraphics[width=1\textwidth]{../cellular_automata/results/rf/rf_importance_all_mse.pdf}
    \caption{Similarity score $\similarity$ sensitivity \label{fig:rfAll}}
\end{subfigure}
%%
\begin{subfigure}[b]{.45\textwidth}
    \includegraphics[width=1\textwidth]{../clustering/results/rf_importance_cluster_mse.pdf}
    \caption{Spread pattern (or cluster membership) sensitivity\label{fig:rfAllCluster}}
\end{subfigure}
%% %%
...
importance. (a)~With similarity~($\similarity$) as the dependent variable, the
analysis was performed on the set of model instances which yielded a
similarity~$\similarity\ge5.5$ ($\approx80$\% match with ground truth). 
...
%% \end{figure}
%%
\paragraph{Domestic trade and and spread within a country.}
