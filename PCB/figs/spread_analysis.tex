...
\node[alg,left of=explore,shift={(0,1)}] (abm) {Multi-pathway model};
\draw[datedge,->] (abm.east) -- +(.3,0) |- ($(explore.west)+(0,.5)$);
\node[measure,left of=explore,shift={(0,-1)}] (score) {Similarity score};
...
%%
\node[blk,right of=cluster,shift={(1,0)}] (analyze) {\textbf{\large Analyse clusters}};
...
{Simulation output of configurations that closely
...
%%
\node[desc,text width=2.7cm,below of=cluster,anchor=north,shift={(1.9,1.2)}] (clustered)
